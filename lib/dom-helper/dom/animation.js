...
exports.default = process.env.BROWSER ? function () {
    var prefix = '',
        eventPrefix;
    ...
    var cssReset = {};
    var transform, supportedHWCompositing, transitionEnd, animationEnd, transitionProperty, transitionOrigin, transitionDuration, transitionTiming, transitionDelay, animationName, animationDuration, animationTiming, animationDelay;
    ...
    if (eventPrefix === undefined && testEl.style.transitionProperty === undefined) {
    ...
    cssReset[transitionOrigin = prefix + 'transition-origin'] = cssReset[transitionProperty = prefix + 'transition-property'] = cssReset[transitionDuration = prefix + 'transition-duration'] = cssReset[transitionDelay = prefix + 'transition-delay'] = cssReset[transitionTiming = prefix + 'transition-timing-function'] = '';
    cssReset[animationName = prefix + 'animation-name'] = cssReset[animationDuration = prefix + 'animation-duration'] = cssReset[animationDelay = prefix + 'animation-delay'] = cssReset[animationTiming = prefix + 'animation-timing-function'] = '';

    function animate(elem, opts) {
        var cssValues = {};
        var ignoreRE = /HWCompositing|property|origin|duration|ease|onComplete|delay/;
        var transforms = '';
        var fired = false;
        var endEvent = void 0,
            cssProperties = void 0,
            _wrappedCallback = void 0;
        ...
        function setStyle(node, name, value) {
            ...
            if (typeof name == "string") {
                if (!value && value !== 0) node.style.setProperty(dasherize(name), value);else node.style[camelize(name)] = maybeAddPx(name, value);
        ...
        _wrappedCallback = function wrappedCallback(event) {
            if (typeof event !== 'undefined') {
                ...
                elem.removeEventListener(endEvent, _wrappedCallback);
        ...
        if (opts.duration > 0) {
            elem.addEventListener(endEvent, _wrappedCallback);
            // transitionEnd is not always firing on older Android phones
            // so make sure it gets fired
            setTimeout(function () {
                if (fired) return;
                _wrappedCallback.call(elem);
